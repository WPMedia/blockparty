...
console.log('the thing', dataTextArea);
//var dataTextArea = document.getRootNode();
...
dataTextArea.addEventListener("drop", function( event ) {
  ...
  uploadAjax.addEventListener('load', function(e) {
  	document.getElementById('renderframe').contentWindow.location.reload();
  });

  uploadAjax.open('POST', '../upload/', true);
  uploadAjax.send(fd);
}, false);
