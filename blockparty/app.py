...
class MongoLoader(jinja2.BaseLoader):
    ...
    def get_source(self, environment, template):
        ...
        google_drive_id = environment.globals[
            'request'].view_args['google_drive_id']
...
client = MongoClient(host=MONGODB_SETTINGS[
...
def upload(id):
    ...
    blocks.find_one_and_update({'_id': ObjectId(id)}, {
...
def view_file(id, filename):
    ...
    if request.method == 'GET':
        ...
        if file:
            ...
            blocks.find_one_and_update({'_id': ObjectId(id)}, {
                                       '$set': {'files': block['files']}}, upsert=True)

    elif request.method == 'POST':
        file = fs.find_one({'filename': id + '/' + filename})
        content = request.form['content']
        oid = fs.put(content, encoding='utf-8',
                     content_type=file.content_type, filename=path)
